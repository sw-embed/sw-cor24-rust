...
pub fn translate_msp430(msp_asm: &str, entry_point: &str) -> Result<String> {
    ...
    let all_labels: Vec<&str> = lines
        .iter()
        .filter_map(|l| {
            if let MspLine::Label(name) = l {
                Some(name.as_str())
            } else {
                None
            }
    ...
    let has_globl = lines
        .iter()
        .any(|l| matches!(l, MspLine::Directive(d) if d.starts_with(".globl")));
    ...
    if has_globl && !has_entry {
        let available: Vec<_> = all_labels
            ...
            .collect();
        bail!(
            "entry point '{}' not found. COR24 convention requires a \
...
fn translate_instruction(inst: &MspInst, call_counter: &mut usize) -> Result<Vec<String>> {
    ...
    match mn {
        ...
        "clrc" => Ok(vec![
            "; clrc (clear carry - handled by shift sequence)".to_string()
        ]),
...
fn translate_binary_op(cor24_op: &str, ops: &[MspOperand], byte_mode: bool) -> Result<Vec<String>> {
    ...
    let (dst, dst_is_spill, dst_working) = if is_spill(&dst_raw) {
        ...
        let w = match &ops[0] {
            MspOperand::Register(r) => match map_register(*r).unwrap_or_default().as_str() {
                "r0" => "r2",
                "r2" => "r0",
                _ => "r0",
            },
...
fn translate_mov(ops: &[MspOperand], byte_mode: bool) -> Result<Vec<String>> {
    ...
    match (&ops[0], &ops[1]) {
        ...
        (MspOperand::Indexed(off, src), MspOperand::Register(dst)) => {
            ...
            let avoid = if is_spill(&d_raw_preview) {
                "r0"
            } else {
                &d_raw_preview
            };
...
fn translate_branch(cor24_branch: &str, ops: &[MspOperand]) -> Result<Vec<String>> {
    match &ops[0] {
        MspOperand::Symbol(target) => Ok(vec![format!("{:<8}{}", cor24_branch, target)]),
...
fn skip_to_ret(lines: &[MspLine], call_idx: usize) -> usize {
    for (j, line) in lines.iter().enumerate().skip(call_idx + 1) {
        if let MspLine::Instruction(inst) = line {
    ...
    match &ops[0] {
        MspOperand::Register(r) => {
            ...
            if is_spill(&reg) {
                ...
                match reg.as_str() {
                    "r0" | "r1" | "r2" | "fp" => Ok(vec![format!("push    {}", reg)]),
                    "sp" => Ok(vec!["; push sp (skipped)".to_string()]),
    ...
    match &ops[0] {
        MspOperand::Register(r) => {
            ...
            if is_spill(&reg) {
                ...
                match reg.as_str() {
                    "r0" | "r1" | "r2" | "fp" => Ok(vec![format!("pop     {}", reg)]),
...
"#;
        ...
        assert!(
            result.contains("push    r2"),
            "should push working reg. Got:\n{}",
            result
        );
        assert!(
            result.contains("lw      r2, 24(fp)"),
            "should load spill_13. Got:\n{}",
            result
        );
        assert!(
            result.contains("add     r0, r2"),
            "add r0 (r12), r2 (spill_13). Got:\n{}",
            result
        );
...
"#;
        ...
        assert!(
            result.contains("and     r0, r2"),
            "and r0, r2 (spill_13). Got:\n{}",
            result
        );
    }
...
"#;
        ...
        assert!(
            result.contains("clu     r0, r2"),
            "clu with spilled r13. Got:\n{}",
            result
        );
...
"#;
        ...
        assert!(
            result.contains("la      r2, mmio_write"),
            "should load call target. Got:\n{}",
            result
        );
        assert!(
            result.contains("jal     r1, (r2)"),
            "should use jal for calls. Got:\n{}",
            result
        );
        assert!(
            result.contains("push    r1"),
            "should save return addr. Got:\n{}",
            result
        );
...
"#;
        ...
        assert!(
            result.contains("sw      r0, 30(fp)"),
            "should save r0 to scratch. Got:\n{}",
            result
        );
        assert!(
            result.contains("lw      r0, 18(fp)"),
            "should load spill_10. Got:\n{}",
            result
        );
        assert!(
            result.contains("push    r0"),
            "should push spill value. Got:\n{}",
            result
        );
        assert!(
            result.contains("lw      r0, 30(fp)"),
            "should restore r0 from scratch. Got:\n{}",
            result
        );
...
"#;
        ...
        assert!(
            result.contains("lw      r0, 24(fp)"),
            "should load spill_13. Got:\n{}",
            result
        );
        assert!(
            result.contains("sw      r0, 21(fp)"),
            "should store to spill_11. Got:\n{}",
            result
        );
        // mov r15, r13: both spilled
        assert!(
            result.contains("lw      r0, 27(fp)"),
            "should load spill_15. Got:\n{}",
            result
        );
        assert!(
            result.contains("sw      r0, 24(fp)"),
            "should store to spill_13. Got:\n{}",
            result
        );
        // ret = jmp (r1)
        assert!(
            result.contains("jmp     (r1)"),
            "ret should use jmp (r1). Got:\n{}",
            result
        );
    }
...
"#
    ...
    fn test_start_convention_produces_prologue() {
        ...
        assert!(
            bra_pos < first_func,
            "bra prologue must precede first function"
        );
    }
...
"#;
        ...
        assert!(
            msg.contains("entry point 'start' not found"),
    ...
    fn test_nonexistent_entry_fails() {
        ...
        let msg = err.to_string();
        assert!(
            msg.contains("entry point 'nonexistent' not found"),
            "Expected 'not found' error, got: {}",
            msg
        );
        assert!(
            msg.contains("start"),
            "Error should list available labels, got: {}",
            msg
        );
    }
...
"#;
        ...
        assert!(
            result.contains("push    r2"),
            "should push working reg r2. Got:\n{}",
            result
        );
        assert!(
            result.contains("lw      r2, 18(fp)"),
            "spill should load into r2. Got:\n{}",
            result
        );
        assert!(
            result.contains("add     r2, r0"),
            "add should use r2 (spill) and r0 (r12). Got:\n{}",
            result
        );
        assert!(
            result.contains("sw      r2, 18(fp)"),
            "result should be stored back. Got:\n{}",
            result
        );
        assert!(
            result.contains("pop     r2"),
            "should pop working reg. Got:\n{}",
            result
        );
    }
...
"#;
        ...
        assert!(
            result.contains("push    r0"),
            "should push dst working reg. Got:\n{}",
            result
        );
        assert!(
            result.contains("lw      r0, 18(fp)"),
        ...
        assert!(
            result.contains("push    r2"),
            "should push src working reg. Got:\n{}",
            result
        );
        assert!(
            result.contains("lw      r2, 24(fp)"),
...
"#;
        ...
        assert!(
            result.contains("; tail call mmio_write"),
            "Should have tail call comment. Got:\n{}",
            result
        );
        assert!(
            result.contains("la      r2, mmio_write"),
            "Should jump to target. Got:\n{}",
            result
        );
        assert!(
            !result.contains("push    r2"),
            "tail call should not push return address. Got:\n{}",
            result
        );
        // Should NOT have pop r2 / jmp (r2) for ret (it was consumed by tail call)
        assert!(
            !result.contains("pop     r2"),
            "tail call should not have ret sequence. Got:\n{}",
            result
        );
    }
...
"#;
        ...
        assert!(
            result.contains("push    r1"),
            "non-tail call should save return addr. Got:\n{}",
            result
        );
        assert!(
            result.contains("jal     r1, (r2)"),
            "non-tail call should use jal. Got:\n{}",
            result
        );
        assert!(
            result.contains("pop     r1"),
            "non-tail call should restore return addr. Got:\n{}",
            result
        );
    }
...
"#;
        let result = translate_msp430(msp430, "start").unwrap();
        assert!(
            !result.contains("Reset vector"),
            "Should not emit prologue without .globl"
        );
    ...
    fn test_prologue_assembles_correctly() {
        ...
        let mut assembler = cor24_assembler::Assembler::new();
    ...
    fn test_echo_v2_rust_logic() {
        ...
        eprintln!(
            "Registers: r0=0x{:06X} r1=0x{:06X} r2=0x{:06X}",
